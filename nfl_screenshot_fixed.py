#!/usr/bin/env python3
"""
NFL.com Mock Draft Screenshot Tool - Fixed Sequential Mapping
Creates Word document with screenshots and correctly mapped analysis text - NO DUPLICATES
"""
...
class NFLScreenshotFixed:
    ...
    def load_page_content(self):
        ...
        for i in range(0, total_height, 2000):
            ...
            time.sleep(0.8)  # Reduced wait time for speed
    ...
    def screenshot_individual_picks(self, author):
        ...
        try:
            ...
            if not pick_elements:
                print(f"   ⚠️ No pick elements found for {author} - trying alternative approach")
                return []
            ...
            for i in range(min(32, len(pick_elements))):
                try:
                    ...
                    time.sleep(0.4)  # Reduced wait for scroll to complete
                    ...
                    time.sleep(0.2)  # Reduced pause between screenshots
    ...
    def create_word_document(self, all_screenshots):
        ...
        doc_path = f"processed/NFL_FIXED_SEQUENTIAL_{timestamp}.docx"
        ...
        try:
            ...
            # Document title
            title = doc.add_heading('2025 NFL Mock Draft - Fixed Sequential Mapping', 0)
            title.alignment = WD_ALIGN_PARAGRAPH.CENTER
            
            subtitle = doc.add_paragraph(f'Generated on {datetime.now().strftime("%B %d, %Y at %I:%M %p")} - NO DUPLICATE DESCRIPTIONS')
            ...
            print(f"\n🎉 SUCCESS! Fixed NFL.com screenshots with sequential mapping!")
            print(f"{'='*55}")
            ...
            print(f"   • Up to 32 picks per author with UNIQUE descriptions")
            print(f"   • Sequential mapping ensures NO DUPLICATES")
            print(f"   • Expert analysis for each pick")
...
def main():
    screenshot_tool = NFLScreenshotFixed()
    
    try:
        if not screenshot_tool.setup_selenium():
            return
        
        print("🎯 Starting NFL Mock Draft Screenshot Collection - FIXED VERSION...")
        print(f"📋 Processing {len(screenshot_tool.authors)} authors with sequential mapping")
        ...
        if all_screenshots:
            print(f"\n📄 Creating fixed Word document with sequential mapping...")
